...
def run_case(case: Case, device: torch.device, args: argparse.Namespace, index: int) -> dict[str, object]:
    ...
    with torch.inference_mode():
        ...
        torch.npu.synchronize()
    impls = {
        "triton": impl_record("triton", "candidate", lambda: run_candidate(inputs), expected, args, case.case_id,
                              profile_for_speed=True),
        "torch": impl_record("torch", "pytorch_semantic_baseline",
                             lambda: reference_impl(*inputs, DEFAULT_EPSILON), expected, args, case.case_id,
                             profile_for_speed=False),
        "torch_npu": impl_record("torch_npu", "task_npu_baseline_probe",
                                 lambda: run_torch_npu(inputs), expected, args, case.case_id,
                                 profile_for_speed=True),
    }
...
def print_record(index: int, total: int, record: dict[str, object]) -> None:
    ...
    print(f"[{status}] {index:03d}/{total:03d} {record['kind']} id={record['case_id']} "
...
def main() -> None:
    ...
    summary: dict[str, object] = {
        ...
        "selected_baseline_counts": {
            name: sum(1 for record in records if record["selected_baseline"]["implementation"] == name)
        ...
        "torch_npu_accuracy_passed": sum(1 for record in records
                                         if record["implementations"]["torch_npu"]["accuracy"]["passed"]),
        ...
        "torch_npu_runnable_all_speed_gate": "N/A" if torch_npu_runnable_all_speedup_geomean is None else (
            "PASS" if torch_npu_runnable_all_speedup_geomean >= 1.2 else "FAIL"),
        "max_rmse": max(float(record["implementations"]["triton"]["accuracy"].get("rmse") or 0.0)
