...
def profile_kernel_details(
    ...
    *,
    ...
    return output, ProfileResult(
        ...
        kernel,
        gap,
        int(metrics["kernel_count"]),
        int(metrics["measured_step_count"]),
        dict(metrics["device_kernels"]),
        {
...
def _failed(error: str, *, prof_dir: Path | None = None, csv_path: Path | None = None,
            trace_path: Path | None = None, wait_info: dict[str, Any] | None = None) -> ProfileResult:
    timeline: dict[str, Any] = {}
    if wait_info is not None:
        timeline["profiler_file_wait"] = wait_info
    return ProfileResult(None, None, None, None, None, None, {}, timeline, str(csv_path) if csv_path else None,
                         str(trace_path) if trace_path else None,
...
def _prepare_warmup_tensors() -> None:
    ...
    if _WARMUP_TENSORS is None:
        ...
        _WARMUP_TENSORS = (
            torch.rand((10240, 10240), dtype=torch.float16, device=device),
            torch.rand((10240, 10240), dtype=torch.float16, device=device),
            torch.rand((96, 1024, 1024), dtype=torch.float16, device=device),
        )
...
def _sync_npu(device: torch.device | None = None) -> None:
    torch.npu.synchronize() if device is None else torch.npu.synchronize(device)
...
def _parse_kernel_timing_csv(csv_path: Path) -> dict[str, Any]:
        ...
        for row in reader:
            ...
            if not step_id:
                ...
                if len(blank_examples) < 3:
                    blank_examples.append({"name": str(name), "type": str(op_type), "start_us": str(start_us),
                                           "duration_us": str(duration)})
                continue
            step_kernel_times.setdefault(step_id, {}).setdefault(name, []).append(duration)
            window = step_windows.setdefault(step_id, {"start_us": start_us, "end_us": start_us + duration,
                                                       "kernel_duration_sum_us": 0.0, "kernel_count": 0})
    ...
    if not step_kernel_times:
        if blank_step_rows:
            return {
                "error_msg": "kernel_details.csv contains only non-warmup kernels without Step Id; "
                             f"blank_step_rows={blank_step_rows}; examples={blank_examples}"
            }
        return {}
    ...
    device_kernels = {name: round(float(statistics.median(times)), 2) for name, times in all_kernel_times.items()}
    active_window_us = float(statistics.median([float(w["device_active_window_us"]) for w in step_windows.values()]))
    median_step_kernel_sum_us = float(statistics.median([float(w["kernel_duration_sum_us"]) for w in step_windows.values()]))
    kernel_counts = [int(w["kernel_count"]) for w in step_windows.values()]
    return {"metrics": {"device_kernels": device_kernels, "kernel_sum_us": round(sum(device_kernels.values()), 2),
                        ...
                        "blank_step_examples": blank_examples}}


def _is_warmup_kernel(op_type: str, input_shapes: str) -> bool:
    return bool(op_type and input_shapes and (
        (op_type == "MatMulV3" and WARMUP_MATMUL_SHAPE in input_shapes) or
        (op_type == "ReduceMax" and WARMUP_REDUCE_SHAPE in input_shapes)
    ))
...
def _locate_kernel_details(prof_dir: Path) -> Path | None:
    for path in [prof_dir / "kernel_details.csv", *prof_dir.rglob("kernel_details.csv")]:
        if path.is_file():
            return path
    return None
...
def _remove_if_quiet(path: str) -> None:
    try:
        ...
        return
    if any(token in text for token in ("error", "exception", "traceback", "failed", "fail ")):
        return
    try:
        Path(path).unlink()
    except OSError:
