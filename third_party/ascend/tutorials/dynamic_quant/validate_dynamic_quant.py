...
def _compare_outputs(
    ...
    return {
        ...
        "mere": scale_mere,
        "mare": scale_mare,
        "rmse": max(output_rmse, scale_rmse),
        "output_rmse": output_rmse,
        "scale_rmse": scale_rmse,
        "max_diff": max(scale_max_abs_diff, float(max_int_diff)),
        "mean_diff": max(scale_mean_abs_diff, output_mean_abs_diff),
        "mismatch_count": mismatch_count + scale_mismatch_count,
        "total_count": int(int_diff.numel() + scale_diff.numel()),
        "error_msg": "" if passed else "integer or scale threshold exceeded",
        "output_results": [
            {
                "name": "output",
...
def _zero_accuracy(outputs: tuple[torch.Tensor, torch.Tensor]) -> dict[str, object]:
    ...
    return {
        ...
        "mean_diff": 0.0,
        "mismatch_count": 0,
        "total_count": int(out.numel() + scale.numel()),
        "error_msg": "",
        "output_results": [
            {
                "name": "output",
...
def _impl_record(
    ...
    *,
    ...
    try:
        ...
        accuracy = (_zero_accuracy(outputs) if role == "pytorch_semantic_baseline" else
                    _compare_outputs(outputs[0], outputs[1], ref_outputs[0], ref_outputs[1]))
...
def build_summary(records: list[dict[str, object]], args: argparse.Namespace) -> dict[str, object]:
    ...
    return {
        ...
        "passed": sum(1 for r in records if acc(r, "triton").get("passed")),
        "failed": sum(1 for r in records if not acc(r, "triton").get("passed")),
        "dst_type_counts": dict(Counter(str(r["dst_type"]) for r in records)),
        "selected_baseline_counts": dict(Counter(str(r["selected_baseline"]["implementation"]) for r in records)),
        "torch_npu_runnable": len(torch_npu_runnable),
        "torch_npu_accuracy_passed": sum(1 for r in records if acc(r, "torch_npu").get("passed")),
        "torch_npu_accuracy_failed": sum(1 for r in torch_npu_runnable if not acc(r, "torch_npu").get("passed")),
        ...
        "candidate_active_geomean_us": _geomean([
            float(r["implementations"]["triton"]["active_window_us"]) for r in records
            if r["implementations"]["triton"].get("active_window_us") is not None
        ]),
        "selected_active_speedup_geomean": _geomean([
            float(r["selected_baseline"]["speedup_vs_triton"]) for r in records
            if r["selected_baseline"].get("speedup_vs_triton") is not None
        ]),
        "max_rmse": max(float(acc(r, "triton").get("rmse") or 0.0) for r in records) if records else 0.0,
        "max_output_rmse": max(float(acc(r, "triton").get("output_rmse") or 0.0) for r in records) if records else 0.0,
        "max_scale_rmse": max(float(acc(r, "triton").get("scale_rmse") or 0.0) for r in records) if records else 0.0,
        "notes": [
...
def main() -> int:
    ...
    with (args.jsonl.open("w", encoding="utf-8") if args.jsonl else open(os.devnull, "w", encoding="utf-8")) as out_file:
