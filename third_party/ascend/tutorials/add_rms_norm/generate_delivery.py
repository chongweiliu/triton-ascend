...
META_BY_DIR = {
    "add_rms_norm": {
        ...
        "design": "Triton-Ascend fused row kernel for H<=8192; wider hidden uses same-backend chunked partial-sum/reduce/apply path.",
    ...
    "dynamic_quant": {
        ...
        "function": "Per-token symmetric dynamic quantization on BF16 [B,S,H], output quantized tensor plus FP32 scale.",
        ...
        "design": "Triton row/row-stride kernel with chunked large-H path and libdevice round before quantized storage.",
...
def find_validation_files(meta: dict[str, Any]) -> tuple[Path, Path]:
    ...
    if not canonical.is_file() or not summary.is_file():
        raise SystemExit(
            ...
            f"--random-seed {meta['random_seed']} --jsonl {meta['validation_jsonl']} "
...
def summary_items(meta: dict[str, Any], records: list[dict[str, Any]], jsonl_path: Path) -> list[tuple[str, Any]]:
    ...
    selected_speedups = [
        fnum((r.get("selected_baseline") or {}).get("speedup_vs_triton")) for r in pubs
        if fnum((r.get("selected_baseline") or {}).get("speedup_vs_triton")) is not None
    ]
    candidate_us = [fnum(impl(r, "triton").get("active_window_us")) for r in pubs if fnum(impl(r, "triton").get("active_window_us"))]
    selected_us = [
        fnum((r.get("selected_baseline") or {}).get("active_window_us")) for r in pubs
    ...
    return [
        ...
        ("overall selected baseline split",
         ", ".join(f"{k}={v}" for k, v in sorted(overall_selected_counts.items())) or "N/A"),
        ...
        ("aux public selected active/active geomean speedup", fmt_x(geomean([v for v in selected_speedups if v is not None]))),
        ("max candidate RMSE", fmt_num(max((fnum(acc(r, "triton").get("rmse")) or 0.0) for r in records) if records else 0.0)),
...
def perf_scope_rows(records: list[dict[str, Any]]) -> list[list[Any]]:
    ...
    return [
        row("main torch_npu timed sample", tnpu_runnable, "torch_npu",
        ...
        row("aux public torch semantic baseline", pubs, "torch", "补充 Torch 语义参考口径，始终作为精度语义基准"),
    ]
...
def write_markdown(meta: dict[str, Any], records: list[dict[str, Any]], jsonl_path: Path, summary: list[tuple[str, Any]]) -> None:
    ...
    design = [
        ...
        "## 4. 性能统计",
        "",
        f"性能统计来自本目录 `run_inference.py --benchmark` 的 torch_npu.profiler kernel_details.csv active-window 计时。主速度验收口径为 `torch_npu runnable all` active/active 几何平均 >= {TORCH_NPU_RUNNABLE_ALL_SPEED_GATE:.1f}x；所有 torch_npu 可计时 case 都纳入，精度通过和失败都计入。selected baseline 仅用于语义标杆选择说明。",
        "",
        "## 5. 统计汇总",
        "",
        table(["指标", "数值"], [[k, v] for k, v in summary]),
    ...
    report = [
        ...
        f"- 速度门槛按 `torch_npu runnable all` active/active 几何平均 >= {TORCH_NPU_RUNNABLE_ALL_SPEED_GATE:.1f}x；无 torch_npu 可计时 case 标记为 N/A，不用 selected baseline 代替。",
...
def write_xlsx(meta: dict[str, Any], records: list[dict[str, Any]], jsonl_path: Path, summary: list[tuple[str, Any]]) -> None:
    ...
    for r in public_rows(records):
        ws.append([
            ...
            fmt_us(impl(r, "triton").get("active_window_us")),
            f"selected={selected_impl(r)}; speedup={fmt_x((r.get('selected_baseline') or {}).get('speedup_vs_triton'))}; RMSE={fmt_num(acc(r, 'triton').get('rmse'))}",
        ])
    style(ws)
    add_sheet(wb, "说明", [["项目", "说明"], ["验证日志", str(jsonl_path)], ["生成命令", REGEN_COMMAND],
                         ["baseline流程", "本目录生成 Torch 与 torch_npu baseline；不读取外部历史评测文件"],
                         ["L1含义", "商业精度等级，不是 L1 norm；本报告仅做本目录自验证指标对齐，不等同完整商业认证"],
                         ["速度门槛", f"torch_npu runnable all active/active 几何平均 >= {TORCH_NPU_RUNNABLE_ALL_SPEED_GATE:.1f}x；无可计时 case 为 N/A"],
                         ["RMSE", "由本目录 checker 输出"]])
...
def write_docx(meta: dict[str, Any], summary: list[tuple[str, Any]], records: list[dict[str, Any]]) -> None:
    ...
    sections = [
        ...
        ("精度标准/性能标准", [f"L1 指标口径参考 references/commercial_standard.md，RMSE/MERE/MARE 由本目录 checker 输出；本报告是教程自验证，不等同完整商业 L1 认证；主速度门槛按 torch_npu runnable all active/active 几何平均 >= {TORCH_NPU_RUNNABLE_ALL_SPEED_GATE:.1f}x，selected baseline 只作语义标杆说明。"]),
        ("Baseline 自包含流程", ["Torch 和 torch_npu baseline 均由本目录 run_inference.py 生成；torch_npu 只有运行成功且精度通过时才被选为 selected baseline；不读取或保留外部历史对照文件。"]),
    ]
...
def main() -> None:
    ...
    print(f"generated {meta['title']}: public={len(public_rows(records))} random={len(random_rows(records))} source={jsonl_path}")
